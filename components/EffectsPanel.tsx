...
export default function EffectsPanel() {
  ...
  const { isLeaving, requestRemoval } = useAnimatedRemoval(removeEffect);
  const { snapshot } = useFlipList(
    listRef,
    effects.map((effect) => effect.instanceId).join('|'),
  );
  ...
  return (
    <>
      ...
      <div className="section-body">
        ...
        <div className="effect-list" ref={listRef}>
          {effects.map((e, i) => {
            const def = getEffect(e.effectId);
            if (!def) return null;
            const leaving = isLeaving(e.instanceId);
            return (
              <div
                key={e.instanceId}
                data-flip-id={e.instanceId}
                className={`effect-card ${e.enabled ? '' : 'disabled'} ${leaving ? 'leaving' : ''}`}
                draggable={!leaving}
              ...
              >
                <div className="effect-card-head">
                  ...
                  <button className="icon-btn" onClick={() => requestRemoval(e.instanceId)}>
